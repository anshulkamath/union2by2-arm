...
int union2by2_test(void)
{
	...
	typedef struct {
		char *name;
		int (*u2b2)(uint16_t *dst, uint16_t *src1, size_t len1,
	...
	for (int i = 0; i < N_funcs; i++) {
		...
		for (int j = 0; j < N_tests; j++) {
			...
			printf("%s/%s/%zu_by_%zu/set_len: ", __FUNCTION__,
			       t.name, n1, n2);
			...
			printf("%s/%s/%zu_by_%zu/forward: ", __FUNCTION__,
			       t.name, n1, n2);
			if ((rc = validate(expected, result, result_size))) {
				break;
			}

			printf("%s/%s/%zu_by_%zu/reverse: ", __FUNCTION__,
			       t.name, n1, n2);
